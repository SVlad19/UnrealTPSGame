...
#include "Tests/TPSCharacterTests.h"
...
const char* CharacterTestBPName = "Blueprint'/Game/Tests/BP_TestThirdPersonCharacter.BP_TestThirdPersonCharacter'";
...
bool FHealthMightBeChangedWithDamage::RunTest(const FString& Parameters)
{
    ...
    TestEqual("Health is full", Character->GetHealthPercent(), 1.f);
...
bool FCharacterDestroyedLatentCommand::Update()
{
    ...
    if (NewTime - StartTime >= LifeSpan)
    {
        ...
        {
            UE_LOG(LogTPSCharacterTests, Error, TEXT("Chracter wasn`t destroyed"));
        }
        return true;
    }
    return false;
